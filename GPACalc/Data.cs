...
namespace GPACalc
{
    class Data
    {
        ...
        /// calculate the gpas
        /// </summary>
        /// <param name="innindatatable">in datatable that contains all the data</param>
        /// <param name="innoutdatatable">out datatable that contains seperate gpa</param>
        /// <param name="innoutoveralldatatable">out datatable that contains overall gpa</param>
        /// <param name="innmethod">the method to calculate with</param>
        ...
        {
            DataTable innindatatable = indatatable.Copy();
            DataTable innoutdatatable = outdatatable.Copy();
            DataTable innoutoveralldatatable = outoveralldatatable.Copy();
            DataTable innmethod = dtmethod.Copy();

            // Store the id of the student to compare with
            long sid = long.Parse(innindatatable.Rows[0][0].ToString());
            ...
            innindatatable.DefaultView.Sort = "Student ID";

            foreach (DataRow dr in innindatatable.Rows)
            {
                if (long.Parse(dr["Student ID"].ToString()) == sid)
                {

                    // Create a new datarow contains all the information of dr and calculate tht gpa
                    DataRow drn = innoutdatatable.NewRow();
                    ...
                    for (int i = 0; i < innmethod.Rows.Count; i++)
                    {
                        if (float.Parse(drn[4].ToString()) >= float.Parse(innmethod.Rows[i][0].ToString()))
                        {
                            drn[5] = innmethod.Rows[i][1];
                            break;
                        }
                    }

                    // add to the datatable
                    innoutdatatable.Rows.Add(drn);
                ...
                else
                {
                    ...
                    innoutoveralldatatable.Rows.Add(drn);
                    ...
                    // Create a new datarow contains all the information of dr and calculate tht gpa
                    DataRow drnn = innoutdatatable.NewRow();
                    ...
                    for (int i = 0; i < innmethod.Rows.Count; i++)
                    {
                        if (float.Parse(drnn[4].ToString()) >= float.Parse(innmethod.Rows[i][0].ToString()))
                        {
                            drnn[5] = innmethod.Rows[i][1];
                            break;
                        }
                    }

                    // add to the datatable
                    innoutdatatable.Rows.Add(drnn);
            ...
            // create a new datarow to store the overall gpa
            DataRow drnl = innoutoveralldatatable.NewRow();

            // copy all the data from original dr to new drn (notice that those info are not in current dr, they are in the last one)
            for (int i = 0; i < 2; i++)
            {
                drnl[i] = innoutdatatable.Rows[innoutdatatable.Rows.Count - 1][i];
            }
            ...
            // add it to the datatable
            innoutoveralldatatable.Rows.Add(drnl);

            outdatatable = innoutdatatable;
            outoveralldatatable = innoutoveralldatatable;
        }
